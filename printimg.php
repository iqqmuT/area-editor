...
elseif (!strcmp($format, "img")) {
    $map = new ImgMap($pois, $areas);
}
...
function get_area_info() {
 ...
 // Arnohack: my CUPS PDF converter doesn't play well with áóéú in file names...
// million thanks to http://php.net/manual/en/function.preg-replace.php#96586
...
function print_title() {
    ...
    if ($area_info) {
 // Arnohack: my CUPS PDF converter doesn't play well with áóéú in file names...
        $title .= "#" . htmlentities(remove_accent($area_info));
    }
    print $title;
}
...
class MapBase {
	...
	if (count($pois)) {
       ...
       $output .= ':::Notes</th><th>Observaciones</th></tr>';
	    foreach ($pois as $poi) {
		$label = $i;
		$i = $i + 1;
	        $output .= '<tr><td>';
...
//       $output .= ':::Notes</th><th>Observaciones</th></tr>';
		...
		$label = $i;
//	        $output .= $poi->notes;
	        $output .= "$label: " . remove_accent(str_replace("\n", " ", $poi->notes)) . "\\n";
           }
        }
//	$output = "<img src=\"notes.jpg\">";
//        return $output;
//        return "XXX $output YYY PRODUCES" . exec("echo -e \"$output\" | /var/www/toe/gensheet.sh");
        $output = escapeshellcmd($output) . "-"; // protect system
//return "image here";
        return "<img src=\"" . exec("echo $output | /var/www/html/toe/gensheet.sh") . "\" alt=\"Cuadrícula de anotaciones\">";
    }
}
...
class GoogleMap extends MapBase {
    ...
    public function handlePOIs() {
		...
		$this->output .= "&markers=label:$label|$color|" . $position; 
	    }
	}
    }
